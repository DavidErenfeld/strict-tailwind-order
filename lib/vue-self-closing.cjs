...
function createOfficialProtectionPrefix(source) {
  let suffix = 0
  let prefix

  do {
    prefix = `StrictTailwindOrderSelfClosing${suffix}Tag`
    suffix += 1
...
function readAttributeValueRanges(source, tag) {
  const ranges = []
  ...
  while (cursor < tag.end - 1) {
    ...
    const nameStart = cursor
    while (cursor < tag.end && !/[\s=/>]/.test(source[cursor])) cursor += 1
    ...
    if (quote === '"' || quote === "'") {
      ...
      ranges.push({name, start: valueStart, end: cursor})
...
function protectVueSelfClosingHtmlForOfficial(source) {
  const replacements = []
  const prefix = createOfficialProtectionPrefix(source)
  let templateDepth = 0
  let cursor = 0
  ...
  while (cursor < source.length) {
    ...
    if (
      ...
      replacements.push({
        ...
        marker: `${prefix}Element${replacements.length}End`,
        original: tag.name
      })

      for (const range of readAttributeValueRanges(source, tag)) {
        if (range.name === 'class' || range.name === ':class' || range.name === 'v-bind:class') continue
        for (let index = range.start; index < range.end; index += 1) {
          if (source[index] !== '&' || !/[#A-Za-z]/.test(source[index + 1] || '')) continue
          replacements.push({
            start: index,
            end: index + 1,
            marker: `${prefix}Entity${replacements.length}End`,
            original: '&'
          })
        }
      }
  ...
  for (const replacement of replacements) {
    protectedSource += source.slice(previousEnd, replacement.start)
    protectedSource += replacement.marker
    previousEnd = replacement.end
  }
  protectedSource += source.slice(previousEnd)

  return {
    text: protectedSource,
    protectedCount: replacements.filter((replacement) => replacement.original !== '&').length,
    restore(value) {
      let restored = value
      const restoreOrder = [...replacements].sort(
        (left, right) => right.marker.length - left.marker.length
      )
      for (const replacement of restoreOrder) {
        restored = restored.replaceAll(replacement.marker, replacement.original)
      }
      return restored
    }
  }
...
module.exports = {
  ...
  protectVueSelfClosingHtmlForOfficial,
  readTag,
